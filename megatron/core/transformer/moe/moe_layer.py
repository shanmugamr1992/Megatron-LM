        ...
        if self.moe_layer_recompute and self.training:
            if self.config.fp8 or self.config.fp4:
                ...
                outputs = tensor_parallel.checkpoint(
                    custom_forward, False, hidden_states, intermediate_tensors, padding_mask, moe_topk_routing_replay_indices
                )
        else:
            outputs = custom_forward(hidden_states, intermediate_tensors, padding_mask, moe_topk_routing_replay_indices)
